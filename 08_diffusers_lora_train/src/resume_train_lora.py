...
def load_existing_lora(pipe):
    if not LORA_OUTPUT_DIR.exists():
        raise FileNotFoundError(
            f"LoRA directory not found: {LORA_OUTPUT_DIR}"
        )

    print(f"Loading existing LoRA: {LORA_OUTPUT_DIR}")
...
def train_one_epoch(
    ...
    for step, batch in enumerate(progress_bar):
        ...
        progress_bar.set_postfix(
            {
                "loss": (
                    f"{loss.item() * GRADIENT_ACCUMULATION_STEPS:.6f}"
                )
            }
        )
...
def main():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        print(
            f"Resume Epoch {epoch + 1}/{NUM_EPOCHS} "
